...
-- | "solve" apenas encapsula a função "_solve"
solve :: Board -> [Board]
solve b = _solve b (0,0) []

{- 
    "_solve" soluciona o tabuleiro propriamente dito, e retorna uma lista
    de soluções válidas. Se a lista retornada for vazia, o tabuleiro não
    possui solução.
-}
_solve :: Board -> (Int, Int) -> [Board] -> [Board]
_solve b (y,x) l 
    | x == (side b)   = _solve b (y+1, 0) l
    | y == (side b)   = b:l
    | emptyAt b (y,x) = _solve' b (y,x) l (getAvailable b (y,x))
    | otherwise       = _solve b (y,x+1) l
    where 
        -- | "_solve' " lida com o backtracking
        _solve' :: Board -> (Int, Int) -> [Board] -> [Int] -> [Board]
        _solve' b (y,x) l []    = l
        _solve' b (y,x) l (h:t) = do
                                    l1 <- [_solve (writeAt b (y,x) h) (y,x+1) l]
                                    _solve' b (y,x) l1 t
...
getRowRegion b (y,x) = let {left' = left b (y,x); right' = right b (y,x)}
                       in getRowRegion' b (minMax(inRowRegion b left' right')) left' right'
    where
...
getColRegion b (y,x) = let {top' = top b (y,x); bottom' = bottom b (y,x)}
                       in getColRegion' b (minMax(inColRegion b top' bottom')) top' bottom'
    where
